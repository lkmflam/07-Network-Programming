...
PORT = 50111              # The same port as used by the server
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:	#Instantiating the socket as s, just like the server.
    ...
    message = input("Please enter the message you would like to send to the server: ")
    m = message.encode()
    s.sendall(m)	
    data = s.recv(1024)
    final = data.decode()	
print('Received', repr(final))	# .repr() is a representation of the string of an object. 
